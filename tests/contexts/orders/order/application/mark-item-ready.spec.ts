...
describe('MarkOrderItemReady', () => {
  let useCase: MarkOrderItemReady
  ...
  beforeEach(() => {
    ...
    useCase = new MarkOrderItemReady(repository, findOrder, eventBus, boardEmitter)
  })
  ...
  it('should not notify the board when the item is not in SENT status', async () => {
    const orderId = UuidMother.random()
    const itemId = UuidMother.random()
    ...
    await expect(useCase.run(orderId, itemId)).rejects.toThrow()
