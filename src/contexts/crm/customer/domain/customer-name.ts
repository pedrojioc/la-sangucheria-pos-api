...
export class CustomerName extends StringValueObject {
  constructor(value: string) {
    super(value)
    if (value.trim().length === 0) throw new InvalidArgument('Customer name cannot be empty')
    if (value.length > 200) throw new InvalidArgument('Customer name cannot exceed 200 characters')
  }
}
