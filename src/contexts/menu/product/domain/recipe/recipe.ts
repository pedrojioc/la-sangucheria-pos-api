...
export interface RecipePrimitives {
  ...
  yield: { value: number; unitId: string; description: string | null }
  isActive: boolean
  createdAt: Date
  updatedAt: Date
}
...
export class Recipe extends AggregateRoot {
  ...
  toPrimitives(): RecipePrimitives {
    return {
      ...
      yield: this._yield.toPrimitives(),
  ...
  static fromPrimitives(primitives: RecipePrimitives): Recipe {
    return new Recipe(
      ...
      RecipeYield.fromPrimitives(primitives.yield),
      primitives.isActive,
