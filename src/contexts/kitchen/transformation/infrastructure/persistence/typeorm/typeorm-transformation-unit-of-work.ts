...
@Injectable()
export class TypeOrmTransformationUnitOfWork extends TransformationUnitOfWork {
  ...
  get transformationRepository(): IngredientTransformationRepository {
    return this.buildTransformationRepository(this.requireManager())
  }

  async commit(work: (uow: TransformationUnitOfWork) => Promise<void>): Promise<void> {
  ...
  private requireManager(): EntityManager {
    if (!this._manager) {
      throw new Error('TransformationUnitOfWork: repositories can only be accessed inside commit()')
    }
  ...
  private buildTransformationRepository(
    manager: EntityManager
  ): IngredientTransformationRepository {
    const repo = manager.getRepository(IngredientTransformationEntity)

    return new (class extends IngredientTransformationRepository {
      async save(transformation: IngredientTransformation): Promise<void> {
        await repo.save(transformation.toPrimitives())
      }

      async search(id: TransformationId): Promise<IngredientTransformation | null> {
        const entity = await repo.findOne({ where: { id: id.value } })
        return entity ? IngredientTransformation.fromPrimitives(entity) : null
      }

      async findByOutputIngredient(
        _ingredientId: IngredientId
      ): Promise<IngredientTransformation[]> {
        return []
      }

      async findByBaseIngredient(_ingredientId: IngredientId): Promise<IngredientTransformation[]> {
        return []
      }
